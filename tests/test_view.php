...
assert_same(
    '<input type="hidden" name="csrf_token" value="0123456789abcdef0123456789abcdef">',
    csrf_input('0123456789abcdef0123456789abcdef'),
    'the hidden field carries the token'
);

// The token is generated by this application, not submitted, so it cannot
// realistically contain markup — but the field is written by concatenation
// rather than by a template, so it escapes anyway. An unescaped value here
// would break out of the attribute and inject markup into every form.
assert_same(
    '<input type="hidden" name="csrf_token" value="&quot;&gt;&lt;script&gt;alert(1)&lt;/script&gt;">',
    csrf_input('"><script>alert(1)</script>'),
    'a token containing markup cannot break out of the value attribute'
);
...
assert_throws(
    static fn () => View::render('catalog/index', [
        ...
        'cartCount' => 0,
        'csrfToken' => 'x',    // supplied, so the failure is unambiguously $cart
    ]),
